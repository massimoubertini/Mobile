...
namespace SamplyGame
{
    public class MassCross : Weapon
    {
        ...
        protected override Task OnFire(bool byPlayer)
        {
            ...
            {
                // angolo per joystick (in radianti)
                var angle = MathHelper.DegreesToRadians(360 / joysticksCount * i);
        ...
        private async Task Fire(Vector3 direction, bool byPlayer)
        {
            ...
            await bulletNode.RunActionsAsync(new MoveBy(5f, direction), new CallFunc(() => bulletNode.SetScale(0f))); //collapse);
            bulletNode.Remove();
        }
    }
}