...
namespace SamplyGame
{
    public class MassMachineGun : Weapon
    {
        ...
        protected override Task OnFire(bool byPlayer)
        {
            ...
            {
                // angolo per proiettile  (in radianti)
                var angle = MathHelper.DegreesToRadians(360 / bulletsCount * i);
                //x^2 + y^2 = length^2 (Equazione del cerchio):
        ...
        private async Task Fire(Vector3 direction, bool byPlayer)
        {
            ...
            await bulletNode.RunActionsAsync(new MoveBy(1f, direction), new CallFunc(() => bulletNode.SetScale(0f)));
            // rimuovere il proiettile dalla scena
            bulletNode.Remove();
        }
    }
}